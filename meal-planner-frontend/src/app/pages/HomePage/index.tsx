...
export function SearchRecipes() {
    const [ingredients, setIngredients] = useState<readonly IngredientAutocompleteItem[]>([])
    const [basicRecipes, setBasicRecipes] = useState<readonly BasicRecipeItem[]>([])
    const [ingredientsSelection, setIngredientsSelection] = useState<IngredientAutocompleteItem[]>([])
    ...
    function handleSelection(e: SyntheticEvent, newValue: IngredientAutocompleteItem[]) {
        setIngredientsSelection(newValue)
    }

    async function handleSearchRecipe(){
        await axios.get("http://localhost:8080/v1/recipe/search?ingredientIds="+ingredientsSelection.map(i => i.id)).then(
            response => (
                setBasicRecipes(response.data.recipes.map((basicRecipe) => {
    ...
    return (
        <Grid2 container direction={"column"}
            ...
            <Grid2>
                {basicRecipes.length > 0 && <BasicRecipeTable inputData={basicRecipes}/>}
            </Grid2>
        </Grid2>
